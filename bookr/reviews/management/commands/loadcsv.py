# FILE PROVIDED BY PACKT / WEB DEV WITH DJANGO
import csv
import re
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for data_dict in models.get("BookContributor", []):
            ...
            contributor = Contributor.objects.get(email=data_dict["book_contributor_contributors"])
            bc, created = BookContributor.objects.get_or_create(
                book=book, contributors=contributor, role=data_dict["book_contributor_role"]
            )
            if created:
